...
public struct PostFixExpression {
    ...
    private func precedence(_ op: String) throws -> Int {
        switch op {
            ...
            default: throw CalculatorCoreError.invalidOperator // Throw error for non-operator characters
        }
    }
    
    /// Function to convert an infix expression to postfix notation
    /// Eg: infix = a + b x c + d; postfix = a b c x + d +
    public func infixToPostfix(_ infix: [String]) throws -> [String] {
        var postFix: [String] = []
        ...
        for elem in infix {
            if elem.isNumber {
                ...
                while let top = stack.peek(), try precedence(elem) <= precedence(top) {
